...
def toggle_fast():
    if not scoreboard.clockState:
        if fastButton.instate(['selected']):
            scoreboard.fast=True
        else:
            scoreboard.fast=False
    else:
        if fastButton.instate(['selected']):
            fastButton.state(['!selected'])
        else:
            fastButton.state(['selected'])
...
setErrorLabel.pack()
clockLabel=Label(controlpanel, text='00.0  ', background='black', foreground='orange')   
...
runButton=Checkbutton(shotclockcontrolpanel, command=shot_run_stop)
runButton.pack()
