...
timeLabel=Label(scoreboard, text=' 0.0 ', bg="black", foreground='red')
timeLabel.grid(column=325,row=0,sticky='N')
homeScoreLabel=Label(scoreboard, text='  0', bg='black', foreground='yellow')
homeScoreLabel.grid(column=0,row=300,sticky='SW')
guestScoreLabel=Label(scoreboard, text='  0', bg='black', foreground='yellow')
...
periodLabel=Label(scoreboard, text='1', bg='black', foreground='red')
periodLabel.grid(column=325, row=300,sticky='S')
periodNameLabel=Label(scoreboard, text='PERIOD', bg='black', foreground='white')
periodNameLabel.grid(column=325,row=290,sticky='S')
homeFoulsNameLabel=Label(scoreboard, text='FOULS', bg='black', foreground='white')
homeFoulsNameLabel.grid(column=310, row=290)
homeFoulsLabel=Label(scoreboard, text='0', bg='black', foreground='yellow')
homeFoulsLabel.grid(column=310, row=300, sticky='SW')
guestFoulsNameLabel=Label(scoreboard, text='FOULS', bg='black', foreground='white')
guestFoulsNameLabel.grid(column=390, row=290)
guestFoulsLabel=Label(scoreboard, text='0', bg='black', foreground='yellow')
guestFoulsLabel.grid(column=390, row=300, sticky='SE')
...
scoreboard.geometry('950x400')
homeFoulsLabel.config(font=('Digital-7 Mono', 75))
guestFoulsLabel.config(font=('Digital-7 Mono', 75))
homeFoulsNameLabel.config(font=('Century Gothic', 25))
guestFoulsNameLabel.config(font=('Century Gothic', 25))
scoreboard.minsize(950, 400)
...
controlpanel.geometry('400x750')
shotclockcontrolpanel.geometry('200x200')
setcontrolpanel.geometry('200x520')
        ...
        while scoreboard.clockState:
            if scoreboard.fast:
                ...
                if shotclock.clockState:
                    ...
                    if (scoreboard.seconds>24 or scoreboard.minutes>0) and shotclock.enabled:
                        time.sleep(0.09)
                    else:
                        time.sleep(0.09)
            ...
            if scoreboard.seconds==0 and scoreboard.clockState and scoreboard.minutes==0 and scoreboard.tenths==0:
                ...
                timeLabel.config(foreground='red')
                scoreboard.update()
                scoreboard.clockState=False
            ...
            if (scoreboard.minutes>0 or scoreboard.seconds>shotclock.seconds) and shotclock.enabled:
                 if shotclock.clockState and (shotclock.seconds>0 or shotclock.tenths>=0):
            ...
            if scoreboard.seconds<10 and scoreboard.minutes>0:
                if scoreboard.minutes<10:
                    ...
                    timeLabel.config(foreground='#00ff00')
                else:
                    timeLabel.config(text=str(scoreboard.minutes)+':0'+str(scoreboard.seconds))
                    clockLabel.config(text=str(scoreboard.minutes)+':0'+str(scoreboard.seconds))
                    mainTimeLabel.config(text=str(scoreboard.minutes)+':0'+str(scoreboard.seconds))
                ...
                if scoreboard.seconds<10:
                    ...
                    timeLabel.config(foreground='yellow')
                else:
                    timeLabel.config(text=str(scoreboard.seconds)+'.'+str(scoreboard.tenths)+' ')
                    clockLabel.config(text=str(scoreboard.seconds)+'.'+str(scoreboard.tenths)+'  ')
                    mainTimeLabel.config(text=str(scoreboard.seconds)+'.'+str(scoreboard.tenths)+' ')
...
def increase_home_score_by_1():
        ...
        if scoreboard.homeScore<10:
            ...
            homeScoreLabel.config(text=str(scoreboard.homeScore))
            homeLabel.config(text=str(scoreboard.homeScore))
        if scoreboard.homeScore>scoreboard.guestScore:
            homeScoreLabel.config(foreground='#00ff00')
            guestScoreLabel.config(foreground='red')
        elif scoreboard.homeScore<scoreboard.guestScore:
            homeScoreLabel.config(foreground='red')
            guestScoreLabel.config(foreground='#00ff00')
        else:
            homeScoreLabel.config(foreground='yellow')
            guestScoreLabel.config(foreground='yellow')
        scoreboard.update()
...
def auto_stop_clock():
    if not scoreboard.autoStopClock:
        ...
        autoStopClockButton.config(text='Automatically stop main clock when shot clock expires: ON')
    else:
        scoreboard.autoStopClock=False
        autoStopClockButton.config(text='Automatically stop main clock when shot clock expires: OFF')
def set_home_name():
    ...
    if not controlpanel.settingHomeFouls:
        ...
        try:
            if homeFoulsEntry.get()=='':
                ...
                controlpanel.settingHomeFouls=False
            elif int(homeFoulsEntry.get())<=9 and int(homeFoulsEntry.get())>=0:
    ...
    if not controlpanel.settingGuestFouls:
        ...
        try:
            if guestFoulsEntry.get()=='':
                ...
                controlpanel.settingGuestFouls=False
            elif int(guestFoulsEntry.get())<=9 and int(guestFoulsEntry.get())>=0:
...
runButton=Scale(shotclockcontrolpanel, orient=HORIZONTAL, sliderlength=40, from_=0, to=1, command=shot_run_stop, state=DISABLED)
runButton.pack()
autoStopClockButton=Button(controlpanel, state=DISABLED, text='Automatically stop main clock when shot clock expires: OFF', command=auto_stop_clock)
autoStopClockButton.pack()
runStopLabel=Label(shotclockcontrolpanel, text='Stop       Run')
runStopLabel.pack()
toggleShotClockButton=Button(controlpanel, text='Enable Shot Clock: OFF', command=toggle_shot_clock)
toggleShotClockButton.pack()
buzzerButton=Button(controlpanel, text='Buzzer', command=buzzer)
buzzerButton.pack()
scoreboard.mainloop()
